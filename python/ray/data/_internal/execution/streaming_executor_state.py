...
class OpBufferQueue:
    ...
    def next_ref_dataset_index(self):
        assert self.has_next()
        return self._queue[0].subdataset_index()
...
class OpState:
    ...
    def __init__(self, op: PhysicalOperator, inqueues: List[OpBufferQueue], cur_dataset_index: int=0):
        ...
        self.cur_dataset_index = cur_dataset_index
        ...
        for i, inqueue in enumerate(self.inqueues):
            if inqueue.has_next() and inqueue.next_ref_dataset_index() == self.cur_dataset_index:
                ...
                if ref is not None:
                    ...
                    return
            elif inqueue.has_next() and inqueue.next_ref_dataset_index() == self.cur_dataset_index + 1:
                has_next_dataset_index = True
        assert has_next_dataset_index, "Nothing to dispatch"

        # update to next_dataset_index
